...
ASSEMBLED_OUTPUT="${1:-$ROOT_DIR/doc/ECOSYSTEM.md}"
...
require_contains "$PARTS_DIR/_index.md" "**Designation:** ECO" "index designation"
require_contains "$PARTS_DIR/_index.md" "BDS Documentation Protocol v2.0" "index protocol"
require_contains "$PARTS_DIR/_index.md" 'Primary output: `doc/ECOSYSTEM.md`' "index primary output"
require_contains "$PARTS_DIR/BUILD.sh" 'DESIGNATION="ECO"' "build designation"
test -f "$ASSEMBLED_OUTPUT"
require_contains "$ASSEMBLED_OUTPUT" "Document version" "assembled document version header"
require_contains "$ASSEMBLED_OUTPUT" "**Designation:** ECO" "assembled designation"
require_contains "$ASSEMBLED_OUTPUT" 'Primary output: `doc/ECOSYSTEM.md`' "assembled primary output"
