...
Current Slice 01 target-role status: Cortex GNAT, FA Local GNAT dispatch,
NeuronForge COR GNAT semantic handoff, and DF Local read-only analytics have
support target-role documents, but runtime, route, schema, migration, model
execution, and durable behavior remain held source-local until later promotion
slices name exact files and proof commands.
